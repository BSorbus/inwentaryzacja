class UserArchivesDatatable < AjaxDatatablesRails::ActiveRecord
  extend Forwardable

  def_delegators :@view, :link_to, :archive_path, :group_path
    ...
    records.map do |record|
      {
        ...
        archivizations: record.archivizations.joins(group: [:users]).where(groups: {users: {id: options[:only_for_current_user_id]}})
          .map {|row| "#{link_to( row.group.fullname, row.group)}: #{row.archivization_type.try(:name)}" }.join('<br>').html_safe
      }
    end
  end
  ...
  private
    ...
    def get_raw_records
      if options[:checked_only_filter].present?
        User.find(options[:only_for_current_user_id]).accesses_archives.distinct
      else
        Archive.all
      end
    end
